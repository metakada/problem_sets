...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarise(MeanImprovementScore = mean(ImprovementScore, na.rm = TRUE),
            SDImprovementScore = sd(ImprovementScore, na.rm = TRUE),
            N_obs = sum(!is.na(ImprovementScore)),#number of observations that do not have NA in them, thought of using length() but realized that using length() would include observations that have NA
            SEM = SDImprovementScore / sqrt(N_obs))
```
